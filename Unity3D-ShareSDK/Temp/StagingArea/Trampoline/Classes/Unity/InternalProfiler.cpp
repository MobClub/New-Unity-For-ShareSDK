...
Profiler_FrameUpdate(const UnityFrameStats* unityFrameStats)
{
	...
	if (_frameId == EachNthFrame)
	{
		...
		::printf("player-detail> physx: %4.1f animation: %4.1f culling %4.1f skinning: %4.1f batching: %4.1f render: %4.1f fixed-update-count: %d .. %d\n",
					   ...
					   MachToMillisecondsDelta((_unityRenderLoopPB.avgV - _dynamicBatchDtPB.avgV - _unityCullingPB.avgV - _unityWaitsForGpuPB.avgV) / EachNthFrame),
#else
					   MachToMillisecondsDelta((_unityRenderLoopPB.avgV - _oglesPB.avgV - _dynamicBatchDtPB.avgV - _unityCullingPB.avgV - _unityWaitsForGpuPB.avgV) / EachNthFrame),
#endif
